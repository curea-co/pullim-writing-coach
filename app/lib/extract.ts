...
// Codex PR #67: channel은 메타데이터(프롬프트에 echo)이므로 화이트리스트로 제한.
//   임의 문자열 허용 시 prompt injection ("file\n이전 지시 무시...") 가능.
//   v2 UniversalCapture 6채널과 일치.
export const ALLOWED_CHANNELS = ["photo", "file", "paste", "link", "voice", "type"] as const;
export type ExtractChannel = (typeof ALLOWED_CHANNELS)[number];
...
export function buildExtractUserPrompt(rawText: string, channel?: ExtractChannel): string {
  const src = channel ? `입력 경로: ${channel}\n` : "";
  return `${src}다음은 선생님이 준 수행평가 안내서 원문이다. 위 스키마의 JSON으로 추출하라.\n\n---\n${rawText}\n---`;
}

// ── 입력 검증 ─────────────────────────────────────────────────────────
export type ExtractRequest = { raw_text: string; channel?: ExtractChannel };
...
function isAllowedChannel(v: unknown): v is ExtractChannel {
...
export function validateExtractRequest(raw: unknown): ExtractValidation {
  ...
  // Codex PR #67: 화이트리스트 외 channel은 silently drop(undefined). 정상 호출자는 영향 0.
  const channel: ExtractChannel | undefined = isAllowedChannel(raw.channel) ? raw.channel : undefined;
