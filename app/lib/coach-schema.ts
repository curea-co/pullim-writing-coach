...
// 공백 제거 정규화 — 인용이 학생 초안의 일부인지(=인용/citation) 강건하게 대조하기 위함.
function squash(s: string): string {
  return s.replace(/\s+/g, "");
}

// 한 nudge(진단+유도질문)에 대필 신호가 있으면 위반. 위반 메시지는 nudge 인덱스를 포함.
//   draft(학생 초안)를 넘기면, 초안에 이미 있는 문장을 코치가 "인용"한 것은 대필로 보지 않는다.
//   (A1 라이브 eval 발견: 코치가 학생 본인 문장을 콕 집어 인용하는 정당한 코칭이 LONG_QUOTE에
//    50% 오탐 → 502로 나가던 버그. 인용 면제로 해결.)
export function checkGenerationBlock(o: CoachOutput, draft = ""): string[] {
  const v: string[] = [];
  const normDraft = squash(draft);
  const isStudentCitation = (snippet: string): boolean =>
    normDraft.length > 0 && normDraft.includes(squash(snippet));
  ...
  (Array.isArray(nudges) ? nudges : []).forEach((n, i) => {
    ...
    // (2) 붙여넣기용 완성문장 인용 — 단, 학생 초안에 있는 문장 인용은 면제(코치의 정당한 지적).
    let quoteHit = false;
    for (const m of text.matchAll(LONG_QUOTE)) {
      const inner = m[1].trim();
      if (SENTENCE_END.test(inner) && !isStudentCitation(inner)) {
        quoteHit = true;
        break;
      }
    }
...
//   draft(학생 초안)를 넘기면 인용 면제가 적용된다(오탐 방지).
export function runCoachGuards(o: CoachOutput, draft = ""): string[] {
  return checkGenerationBlock(o, draft);
}
