...
export function tabItems(pathname: string): RailItem[] {
  // Mobile bottom bar: the 5 primary content routes (홈/채점받기/코치/결과/샘플).
  // 내 정보·서비스 소개 stay rail-only (reachable on mobile via the home banner/closing CTA).
  return NAV.slice(0, 5).map((n) => ({
    label: TAB_LABELS[n.href] ?? n.label,
    href: n.href,
    icon: NAV_ICONS[n.href],
    active: isActive(n.href, pathname),
  }));
}
