...
it("authed 삭제 — DELETE 실패(5xx) 시 이동 안 함 + 에러 안내(결함 3)", async () => {
  // PR #115 결함 3: DELETE 응답을 확인 — 실패면 로컬 정리·이동 금지, 에러 안내만.
  authStatus = "authed";
  globalThis.fetch = vi.fn().mockResolvedValue({ status: 500, json: async () => ({}) });
  const Page = (await import("@/app/me/page")).default;
  render(<Page />);
  await clickDeleteFlow();
  await screen.findByText("삭제에 실패했어요. 다시 시도해 주세요.");
  expect(routerPush).not.toHaveBeenCalled();
  expect(clearAllLocalStorage).not.toHaveBeenCalled();
});

it("authed 삭제 — DELETE 네트워크 오류 시 이동 안 함 + 에러 안내(결함 3)", async () => {
  authStatus = "authed";
