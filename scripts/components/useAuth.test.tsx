...
it("login: csrf 토큰 못 받으면 자격증명 미전송 + 연결오류 메시지", async () => {
  let loginCalled = 0;
  (globalThis.fetch as any).mockImplementation((url: string) => {
    if (String(url).includes("/api/auth/me")) return Promise.resolve({ ok: true, json: async () => ({ authenticated: false }) });
    if (String(url).includes("/api/auth/csrf")) return Promise.resolve({ ok: false, status: 500, json: async () => ({}) }); // csrf 장애 → 토큰 없음
    if (String(url).includes("/api/auth/login")) { loginCalled++; return Promise.resolve({ ok: true, json: async () => ({}) }); }
  ...
  expect(result?.message).toMatch(/보안 토큰/);
  expect(loginCalled).toBe(0); // 자격증명 전송 안 함
});
