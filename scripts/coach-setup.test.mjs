...
test("parseSetup — 손상된 title(문자열 아님)은 버리고 나머지 과제는 보존(TypeError 방지)", () => {
  const base = { school_level: "중2", subject: "과학", genre: "설명문", target_char_count: null, prompt_text: "화산의 형성을 설명하라" };
  // title: 숫자 → 드롭(undefined), 나머지 보존
  const numTitle = parseSetup(JSON.stringify({ assignment: { ...base, title: 123 }, mode: "free" }));
  assert.deepEqual(numTitle, { assignment: { ...base, title: undefined }, mode: "free" });
  // title: 객체 → 드롭
  const objTitle = parseSetup(JSON.stringify({ assignment: { ...base, title: { x: 1 } }, mode: "free" }));
  assert.equal(objTitle.assignment.title, undefined);
  assert.equal(objTitle.assignment.prompt_text, base.prompt_text); // 과제 보존
  // 정상 문자열 title은 그대로 보존
  const okTitle = parseSetup(JSON.stringify({ assignment: { ...base, title: "화산" }, mode: "free" }));
  assert.equal(okTitle.assignment.title, "화산");
});
