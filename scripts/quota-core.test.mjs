...
test("한도 상수: score 1회(1일 1회) · coach 4회(세션=초기1+재점검3, #134 UX 환산)", () => {
  assert.equal(FREE_DAILY_LIMITS.score, 1);
  assert.equal(FREE_DAILY_LIMITS.coach, 4);
});
...
test("isQuotaExceeded: score는 오늘 1회면 초과, coach는 4회부터 초과", () => {
  const today = "2026-07-09";
  assert.equal(isQuotaExceeded({ [QUOTA_FIELD]: { score: { d: today, n: 1 } } }, "score", today), true);
  assert.equal(isQuotaExceeded({ [QUOTA_FIELD]: { coach: { d: today, n: 3 } } }, "coach", today), false);
  assert.equal(isQuotaExceeded({ [QUOTA_FIELD]: { coach: { d: today, n: 4 } } }, "coach", today), true);
});
