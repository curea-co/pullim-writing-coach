...
const MUST_BLOCK = [
  ...
  // 우회 차단: 인용 안에 '~'를 끼워 넣어도 인용이 '를 넣' 직접 목적어면 그대로 잡힌다(리뷰 반영).
  ["질문형 covert: 인용 내 '~' 우회 시도", "'화산은 위험하다 ~'를 넣어봐"],
];
...
const MUST_PASS = [
  ...
  // 연결어 템플릿(coach-mock): 인용 뒤가 '처럼'이라 '를 넣' 직접목적어 아님 → 통과(별도 '~' 예외 불필요).
  ["연결어 템플릿(coach-mock)", "'먼저 ~, 그래서 ~, 따라서 ~'처럼 이어 주는 말을 넣어 볼까요?"],
  // 학생 자기 문장 echo+보강: 인용은 학생 문장, '를' 뒤에 '더 자세히'가 끼어 직접 삽입 아님 → 통과(리뷰 반영).
  ["학생 문장 echo+보강", "네가 쓴 '나는 화산이 무섭다'를 더 자세히 넣어 설명해 볼까?"],
  // 학생 장면 인용이 주어(는)이고 삽입동사가 분리 → 통과(리뷰 반영).
  ["학생 장면 인용(주어)+분리 넣어", "'동생이랑 바다에 갔다'는 좋은 장면이야, 더 넣어 볼까?"],
];
