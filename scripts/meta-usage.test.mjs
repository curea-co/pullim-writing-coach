...
test("초기 상태 — loadMetaUsage 빈 객체, getMostUsedMeta null", async () => {
  const usage = await loadMetaUsage();
  assert.deepEqual(usage, { school_level: [], subject: [], genre: [], target_raw: [] });
  assert.equal(await getMostUsedMeta("genre"), null);
});

test("recordMetaUsage 1회 — count=1, last_used_at 설정", async () => {
  await recordMetaUsage("genre", "설명문");
  const usage = await loadMetaUsage();
...
test("같은 값 반복 기록 — count 누적, last_used_at 갱신", async () => {
  await recordMetaUsage("genre", "설명문");
  await recordMetaUsage("genre", "설명문");
  await recordMetaUsage("genre", "설명문");
  const usage = await loadMetaUsage();
...
test("getMostUsedMeta — count 최빈값 반환", async () => {
  await recordMetaUsage("genre", "설명문");
  await recordMetaUsage("genre", "설명문");
  await recordMetaUsage("genre", "설명문"); // count=3
  await recordMetaUsage("genre", "감상문·독후감"); // count=1
  await recordMetaUsage("genre", "논설문·주장하는 글"); // count=1
  assert.equal(await getMostUsedMeta("genre"), "설명문");
});

test("getMostUsedMeta — count 동률 + last_used_at 동률 시 stable sort(insertion 순)", async () => {
  // 같은 millisecond에 두 값 기록 시 timestamp 동률. Array.sort stable이라 첫 추가가 선두 유지.
  // 실제 사용자 시간 흐름에서는 의미 있는 차이 발생 → 최신 우선 자연 동작.
  await recordMetaUsage("genre", "설명문");
  await recordMetaUsage("genre", "감상문·독후감");
  const top = await getMostUsedMeta("genre");
  // 어느 쪽이든 둘 중 하나 — 핵심은 가장 최근(또는 stable) 값 반환, null 아님.
  assert.ok(top === "설명문" || top === "감상문·독후감");
});

test("LRU 5건 제한 — 6번째 신규 추가 시 가장 오래된 1건 drop", async () => {
  // 5개 unique 값 기록 (서로 다른 시각 — recordMetaUsage 호출 간 미세 시차)
  await recordMetaUsage("genre", "A");
  await recordMetaUsage("genre", "B");
  await recordMetaUsage("genre", "C");
  await recordMetaUsage("genre", "D");
  await recordMetaUsage("genre", "E");
  let usage = await loadMetaUsage();
  ...
  // 대신 max 길이 유지 검증.
  await recordMetaUsage("genre", "F");
  usage = await loadMetaUsage();
  assert.equal(usage.genre.length, MAX_META_USAGE_PER_FIELD);
  // F는 추가됐어야 함
  assert.ok(usage.genre.some((e) => e.value === "F"));
});

test("빈 문자열·공백 — 기록 안 함(가드)", async () => {
  await recordMetaUsage("genre", "");
  await recordMetaUsage("genre", "   ");
  const usage = await loadMetaUsage();
  assert.equal(usage.genre.length, 0);
});

test("필드 독립성 — school_level 추가가 genre에 영향 X", async () => {
  await recordMetaUsage("school_level", "중2");
  await recordMetaUsage("subject", "국어");
  await recordMetaUsage("genre", "설명문");
  await recordMetaUsage("target_raw", "800");
  const usage = await loadMetaUsage();
...
test("clearMetaUsage — 전체 초기화", async () => {
  await recordMetaUsage("genre", "설명문");
  await clearMetaUsage();
  assert.equal(await getMostUsedMeta("genre"), null);
});

test("loadMetaUsage — 손상된 JSON → 빈 객체 폴백", async () => {
  mem.set("pwc_meta_usage_v1", "{not-json");
  const usage = await loadMetaUsage();
  assert.deepEqual(usage, { school_level: [], subject: [], genre: [], target_raw: [] });
});

test("loadMetaUsage — 스키마 위반 → 빈 객체 폴백", async () => {
  mem.set("pwc_meta_usage_v1", JSON.stringify({ wrong: "structure" }));
  const usage = await loadMetaUsage();
  assert.equal(usage.genre.length, 0);
});

test("loadMetaUsage — 손상 엔트리 1건은 drop하고 나머지 유효 이력은 보존(필드별/엔트리별 정제)", async () => {
...
test("loadMetaUsage — 중복 value dedup (count 합산, last_used_at 더 최신)", async () => {
...
test("loadMetaUsage — 6건 이상 유효 엔트리도 LRU 5건 cap 복원", async () => {
...
test("loadValidatedMetaUsage — 최신 N건이 enum 외(손상)여도 정상 이력 살아남음 (cap before enum 회귀 검증)", async () => {
  ...
  const usage = await loadValidatedMetaUsage();
  assert.equal(usage.subject.length, 1, "정상 1건만 남아야 함");
  assert.equal(usage.subject[0].value, "국어");
});

test("recordMetaUsage — quota 초과 시 throw 안 함(silent)", async () => {
  mode = "quota";
  // 첫 호출은 LS read는 되지만 write quota에서 실패 — throw 없이 silent.
  await assert.doesNotReject(() => recordMetaUsage("genre", "설명문"));
});
