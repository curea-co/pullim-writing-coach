...
// Codex #155 10R — 안전 우선 결정: 같은 nudge의 반대 필드에 재작성 지시가 있으면(설령 인용과
//   무관한 다른 문장이어도) echo 면제를 아예 거부한다. 5R/8R이 고친 "동일 필드 내 지역 오탐"과는
//   다른 계층 — 대필 불변식이 걸린 사안이라, 드문 우연의 오탐(무관한 반대 필드 표현)보다 교차 필드
//   분산 우회(진짜 대필 유도, 아래 별도 테스트) 미탐지가 더 위험하다고 판단(Codex 명시 요구).
test("반대 필드에 재작성 지시가 있으면 그 문장이 인용과 무관해도 echo 면제 거부(10R 안전 우선)", () => {
  const o = validOutput();
  const sentence = "학생들이 자신의 개성을 표현할 수 있기 때문이다.";
  const draft = `나는 교복 자율화에 찬성한다. ${sentence} 또한 편하다.`;
...
// 대칭 확인 — 인용 앞의 재작성 지시가 "다른 문장"(문장 경계 너머)에 있어도 10R 정책상 여전히 거부.
test("재작성 지시가 인용 앞의 다른 문장(문장 경계 너머)에 있어도 반대 필드면 echo 면제 거부(10R)", () => {
  const o = validOutput();
  const sentence = "학생들이 자신의 개성을 표현할 수 있기 때문이다.";
  const draft = `나는 교복 자율화에 찬성한다. ${sentence} 또한 편하다.`;
...
// Codex #155 8R(동일 필드 격리)은 유지되지만, 10R(반대 필드 전역 검사)이 그 위에 얹혀서 "반대 필드에
//   재작성 지시가 있으면 문장부호 유무와 무관하게" 여전히 거부한다 — 8R이 막던 건 "같은 필드 안"의
//   지역 오탐(예: 인용 자체가 있는 필드에서 먼 무관 문장)이지 반대 필드 전역 검사가 아니다.
test("diagnosis가 마침표 없이 끝나도, 반대 필드(diagnosis)에 재작성 지시가 있으면 echo 면제 거부(10R)", () => {
  const o = validOutput();
  const sentence = "학생들이 자신의 개성을 표현할 수 있기 때문이다.";
  const draft = `나는 교복 자율화에 찬성한다. ${sentence} 또한 편하다.`;
...
// Codex #155 10R — 9R에서 known_gap으로 남겼던 교차 필드 우회를 안전 우선으로 닫았다: 인용이
//   있는 필드가 echo 검증을 통과해도, **반대 필드 어디에든** 재작성 지시가 있으면 면제를 거부한다.
//   대필 불변식이 걸린 사안이라 드문 우연의 오탐(위 10R 테스트들)보다 이 미탐지가 더 위험하다는
//   Codex의 명시적 판단을 반영 — known_gap에서 must_block류로 승격.
test("인용(diagnosis)·재작성 지시(question) 교차 필드 분산 우회 차단(10R — 9R known_gap 승격)", () => {
  const o = validOutput();
  const sentence = "학생들이 자신의 개성을 표현할 수 있기 때문이다.";
  const draft = `나는 교복 자율화에 찬성한다. ${sentence} 또한 편하다.`;
