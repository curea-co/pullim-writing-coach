...
test("메타테스트 (a): 알려진 대필 문자열은 반드시 throw", () => {
  ...
  // 질문+목록 혼합 떠먹임 (회귀 가드, 리뷰 반영): '?'가 있으면 listHandsOverContent가 통과시키던 사각.
  //   ?-제거 사본을 함께 검사하므로 이제 잡혀야 한다.
  assert.throws(
    () => assertNoGeneration(["두 가지를 써볼까? 1) 화산은 위험하다 2) 대피가 필요하다"], "질문+목록혼합"),
    Error,
    "질문+목록 혼합 떠먹임이 throw 안 함"
  );
});
