...
// ── 라운드트립 ────────────────────────────────────────────────
test("미존재 키 → emptyConsent 폴백", () => {
  assert.deepEqual(loadConsent(), emptyConsent());
});

test("save → load 라운드트립", () => {
  const state = { serviceConsentAt: TS, aiTrainingOptInAt: null, guardianConsentAt: TS };
  assert.deepEqual(saveConsent(state), { ok: true });
  assert.deepEqual(loadConsent(), state);
});

test("키 = 'pwc-consent-v1' (공유 계약)", () => {
  saveConsent(emptyConsent());
  assert.ok(storageMock.getItem("pwc-consent-v1") !== null);
});

test("손상 JSON → emptyConsent 폴백", () => {
  storageMock.setItem("pwc-consent-v1", "{ broken");
  assert.deepEqual(loadConsent(), emptyConsent());
});

test("schema 손상(숫자 필드) → emptyConsent 폴백", () => {
  storageMock.setItem("pwc-consent-v1", JSON.stringify({ serviceConsentAt: 1 }));
  assert.deepEqual(loadConsent(), emptyConsent());
});

test("clearConsent → 다시 emptyConsent", () => {
  saveConsent({ serviceConsentAt: TS, aiTrainingOptInAt: TS, guardianConsentAt: null });
  clearConsent();
  assert.deepEqual(loadConsent(), emptyConsent());
});
...
test("AI 옵트인 기본 OFF — emptyConsent는 mayUseForAiTraining false", () => {
  assert.equal(mayUseForAiTraining(loadConsent()), false);
});

test("setAiTrainingOptIn(true) → mayUseForAiTraining true (영속 후에도)", () => {
  const on = setAiTrainingOptIn(emptyConsent(), true, TS);
  saveConsent(on);
  assert.equal(mayUseForAiTraining(loadConsent()), true);
});

test("setAiTrainingOptIn(false) 철회 → null → mayUseForAiTraining 다시 false", () => {
