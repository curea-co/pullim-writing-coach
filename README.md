...
> 🔗 **데모**: https://pullim-writing-coach-demo.vercel.app
>
> 데모 비밀번호 입력 후 과정 코치(`/coach`)를 체험하거나, 5종 학생 글 샘플(`/samples`)의 채점 결과를 미리 볼 수 있습니다.
>
...
2. 코치 셋업 (프로필·연령 동의)
  ↓
3. 수행평가 안내서 입력 (6채널 캡처) → AI 추출 → 안내서 카드(신뢰도 칩·인라인 수정)
  ↓
4. 모드 선택
   ├─ 이미 글 있어요  → 작성 캔버스 → 채점
   ├─ 아직 안 썼어요  → 장르별 가이드 질문 → 본문 작성 → 채점
   └─ 개요 먼저       → 개요 패널 → 개요→본문 전환 → 채점
  ↓
...
```bash
npm install
cp .env.example .env   # 키 입력 (빈 값이면 UI는 뜨지만 라이브 API는 동작 안 함)
